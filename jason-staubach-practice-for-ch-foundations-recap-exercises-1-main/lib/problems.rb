...
def all_vowel_pairs(words)
  ...
  words.each_with_index do |word1, i|
    (i + 1 ...words.length).each do |j|
      vCheck = {"a"=> false, "e" => false, "i"=> false, "o"=> false, "u" => false}
      word1.each_char {|char| vCheck[char] = true if vowels.include?(char)}
      words[j].each_char {|char| vCheck[char] = true if vowels.include?(char)}
      if !vCheck.has_value?(false)
        vowelPair << word1 + " " + words[j]
      end
    end
  end
  vowelPair
